...
"""
Embed missing vectors via backend /query API using MiniLM helper (local or HF HTTP).

Usage (auto provider):
    HF_TOKEN=... python -m jobs.embed_chunks --backend http://localhost:8000 --table chunks --limit 200 --batch-size 64

Local-only (if torch/sbert installed):
    MINILM_PROVIDER=local python -m jobs.embed_chunks --table chunks

HTTP-only (no Python embedding wheels needed):
    MINILM_PROVIDER=hf HF_TOKEN=... python -m jobs.embed_chunks --table chunks
"""
...
def _fetch_missing_batch(
    ...
    return [row for row in data if text_field in row]


def _update_embedding_row(backend: str, table: str, rid_value, payload: dict):
    body = {"action": "update", "table": table, "rid": rid_value, "payload": payload}
...
def run(
    ...
    while processed < limit:
        ...
        if provider:
            os.environ["MINILM_PROVIDER"] = provider
        ...
        for r, vec in zip(rows, vecs):
            ...
            if dry_run:
                print({"table": table, "rid": rid, "set": embed_field, "dim": len(vec), "provider": provider or "auto"})
            else:
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    p.add_argument("--provider", choices=["auto", "local", "hf"], default=os.environ.get("MINILM_PROVIDER", "auto"))
